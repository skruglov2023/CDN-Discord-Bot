...
class AutomatedStuff(commands.Cog):
    ...
    async def on_message_delete(self, message):
        ...
        embed = discord.Embed(title = f"{author}'s message in {this_channel} was deleted by {deleter}",
                              description = "",
    ...
    async def on_message(self, message):
        ...
        if message.author.id is not stephan.id:
            ...
            if "soufflé" in msg and message.author.id is aj.id:
                ...
                await message.channel.send(
                    f"I\'m getting real tired of filtering your variations for my name, {aj.mention}")
                return
            for word in notStephan_sName:
                ...
                if word in msg_strip:
                    await message.delete()
                    await message.channel.send(
                        f"It's still stephan to you {message.author.mention}! maybe also stop trying to avoid my filter...",
                        delete_after = 10)
        ...
        a_emoji_names = re.findall(r'a:\w*:',
                                   str(a_custom_emojis))  # find discord emoji name in the list of custom emoji
    ...
    async def on_raw_reaction_add(self, reaction):
        ...
        # print(f"{eid}, {ename}") print(f"reaction channel: {channel}, bot testing channel: {bot_testing}") if
        # channel==bot_testing: await message_id.remove_reaction(emoji, userid) print(f"removed reaction {eid},
        # {ename}") await bot_testing.send(f"{eid}/{ename} reaction from {userid.display_name} was removed due to
        # lack of roles")
